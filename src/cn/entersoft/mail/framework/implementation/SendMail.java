...
public class SendMail extends MailPort{
	...
	/**
	 * 有参构造器，在创建SendMail对象的时候链接邮箱服务器，并且创建MimeMessage对象,同时生成transport对象
	 * @param host
	 * @param userName
	 * @param passWord
	 * @param validate
	 * @param deBug
	 */
	public SendMail(Session session,String protocol) {
	try {
...
/*	public static SendMail getDefaultSendMail(Session session) {
	...
	public boolean send(){
		try {
			...
			this.	transport.sendMessage(message,message.getRecipients(RecipientType.TO));
 ...
 */
	...
	public boolean setImg(MimeMultipart bodyMultipart) {
		/** foeach 循环处理正文中的图片 */
		for (Entry<String, String> entry : this.imgMap.entrySet()) {
			try {
				/** 创建MimeBodyPart对象 */
	 ...
	 */
	public boolean setAttach() {
		/** 循环处理附件 */
		for (Entry<String, String> entry : this.attachMap.entrySet()) {
			try {
				/** 创建MimeBodyPart对象 */
